...
class Panel {
  ...
  init() {
    function generatePane() {
      ...
      sn.addInput(config.scene, "speed", {
        min: 0,
        max: 1,
        label: "Speed",
      });
      const ob = pn.addFolder({ title: "Object" });
      ob.addInput(config.object, "speed", {
        min: 0,
        max: 1,
        label: "Speed",
      });
...
class LightBar {
  ...
  light(e, m) {
    ...
    this.p_light.position.set(m.position);
    e.add(this.p_light);
  }
}
...
class Space {
  ...
  object() {
    ...
    this.verticalMirror = new Reflector(c_geo, {
      clipBias: 0.005,
    ...
    this.verticalMirror.position.set(1, 1, 0);
